...
Java_jcarbon_rapl_Rapl_readNative(JNIEnv *env, jclass jcls) {
	char energy_str[512];
...
Java_jcarbon_rapl_Rapl_wrapAround(JNIEnv * env, jclass jcls) {
...
JNIEXPORT jdouble JNICALL
Java_jcarbon_rapl_Rapl_dramWrapAround(JNIEnv * env, jclass jcls) {
  int fd = open("/dev/cpu/0/msr",O_RDONLY);
...
JNIEXPORT jstring JNICALL
Java_jcarbon_rapl_Rapl_components(JNIEnv* env, jclass jcls) {
	char* order;
